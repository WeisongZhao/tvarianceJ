...
import signaltvJ.RealSignal;
import signaltvJ.SignalCollector;

public class tvarianceJ_ extends JDialog implements PlugIn {
	private static int skip = 10;
	private static int rollfactor = 10;
	...
	public void run(String arg) {
		...
		String titleImage = Prefs.get("tvariance.titleImage", titles[0]);
		int imageChoice = 0;
		for (int i = 0; i < wList.length; i++) {
		...
		gd.addHelp("https://github.com/WeisongZhao/tvarianceJ");
		gd.showDialog();
		if (gd.wasCanceled())
			return;
		// Get parameters
		...
		if (!showDialog())
			return;
		SOFI_recon(impY, skip, 2, rollfactor);
	}
	...
	public void SOFI_recon(ImagePlus imp, int skip, int order, int rollfactor) {
	...
	protected void dealWithTimePointFrame(int f, ImagePlus cum) {
		...
		if (f == 0) {
			...
			impReconstruction = new ImagePlus("t-variance result", imsReconstruction);
			impReconstruction.show();
			Apply_LUT_TVJ.applyLUT_redhot(impReconstruction);
		}
		else {
